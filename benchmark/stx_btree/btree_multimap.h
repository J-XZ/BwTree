...
class btree_multimap {
 ...
 public:
  ...
  typedef stx::btree<key_type, data_type, value_type, key_compare, traits, true, allocator_type,
                     false>
      btree_impl;
 ...
 public:
  ...
  /// comparison function
  explicit inline btree_multimap(const allocator_type &alloc = allocator_type()) : tree(alloc) {}
 ...
 public:
  ...
  inline iterator insert(const value_type &x) { return tree.insert2(x.first, x.second).first; }
 ...
 public:
  ...
  // 删除与给定键关联的所有键/数据对。这是使用 erase_one() 实现的，因此效率不高。
  size_type erase(const key_type &key) { return tree.erase(key); }

  /// Erase the key/data pair referenced by the iterator.
  void erase(iterator iter) { tree.erase(iter); }
 ...
 public:
  ...
  size_t GetMemoryFootprint() const { return tree.GetMemoryFootprint(); }
};
