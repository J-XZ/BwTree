...
struct btree_default_set_traits {
  ...
  /// printable.
  // 如果为 true，则在执行 insert() 或 erase() 操作时，树会打印调试信息和树的结构信息。
  // 前提是头文件必须在定义了 BTREE_DEBUG 的情况下编译，并且 key_type 必须支持通过 std::ostream 输出。
  // #ifdef BTREE_DEBUG
...
template <typename _Key, typename _Data, typename _Value = std::pair<_Key, _Data>,
          ...
          typename _Traits = btree_default_map_traits<_Key, _Data>, bool _Duplicates = false,
          typename _Alloc = std::allocator<_Value>, bool _UsedAsSet = false>
class btree {
 public:
  ...
  // 第八个模板参数：布尔值，指示 btree
  // 是否用作集合（set）。在这种情况下，
  // 会省略对数据数组的所有操作。这个标志有点hack，但是必要的，因为根据C标准，
  // sizeof(empty_struct) = 1。如果没有这个标志，就会发生大量多余的复制。
 ...
 public:
  ...
  typedef btree<key_type, data_type, value_type, key_compare, traits, allow_duplicates,
 ...
 private:
  ...
  struct node {
    ...
    // inline unsigned short GetSlotUse() const { return slotuse; }
  };
  ...
  struct inner_node : public node {
    ...
    // inline unsigned short GetSlotUse() const { return node::GetSlotUse(); }
  };
 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    inline iterator(typename btree::leaf_node *l, unsigned short s) : currnode(l), currslot(s) {}

    /// Copy-constructor from a reverse iterator
    inline iterator(const reverse_iterator &it) : currnode(it.currnode), currslot(it.currslot) {}
    ...
    inline data_type &data() const { return currnode->slotdata[used_as_set ? 0 : currslot]; }
  ...
  class const_iterator {
   ...
   public:
    ...
    inline const_iterator(const iterator &it) : currnode(it.currnode), currslot(it.currslot) {}
    ...
    inline const data_type &data() const { return currnode->slotdata[used_as_set ? 0 : currslot]; }
  ...
  class reverse_iterator {
   ...
   public:
    ...
    /// Copy-constructor from a mutable iterator
    inline reverse_iterator(const iterator &it) : currnode(it.currnode), currslot(it.currslot) {}
 ...
 public:
  ...
  /// comparison object
  explicit inline btree(const key_compare &kcf, const allocator_type &alloc = allocator_type())
      : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_key_less(kcf), m_allocator(alloc) {}
 ...
 private:
  ...
  inline bool key_less(const key_type &a, const key_type b) const { return m_key_less(a, b); }

  /// True if a <= b ? constructed from key_less()
  inline bool key_lessequal(const key_type &a, const key_type b) const { return !m_key_less(b, a); }

  /// True if a > b ? constructed from key_less()
  inline bool key_greater(const key_type &a, const key_type &b) const { return m_key_less(b, a); }
 ...
 public:
  ...
  inline iterator end() { return iterator(m_tailleaf, m_tailleaf ? m_tailleaf->slotuse : 0); }
 ...
 private:
  ...
  // 检索节点n中第一个大于或等于key的键。
  // 使用二进制搜索和可选的线性自我验证。
  // 这是一个模板函数，因为slotkey数组位于leaf_node和inner_node中的不同位置。
  //
  // 这个函数总是在一个有序的数组中查找一个大于或等于给定键的第一个位置。
  // 它返回一个整数索引，表示该位置在节点的slotkey数组中的位置。
  template <typename node_type>
  inline int find_lower(const node_type *n, const key_type &key) const {
    // 对于key数量较多的节点，使用二分查找，其余情况使用线性查找
 ...
 public:
  ...
  iterator find(const key_type &key) {
    ...
    int slot = find_lower(leaf, key);
    return (slot < leaf->slotuse && key_equal(key, leaf->slotkey[slot])) ? iterator(leaf, slot)
                                                                         : end();
  }
 ...
 public:
  ...
  inline bool operator!=(const btree_self &other) const { return !(*this == other); }
 ...
 private:
  ...
  std::pair<iterator, bool> insert_start(const key_type &key, const data_type &value) {
    ...
    // 如果 newchild 不为空，说明根节点被分裂了，需要创建一个新的根节点
    if (newchild) {
      // 根节点总是只有1个，如果根节点满了，总是1分为2，因此新的根节点总是只有2个子节点

      inner_node *newroot = allocate_inner(m_root->level + 1);  // 每个新根节点的层级比旧根节点高1
      // 根节点的level是最高的，叶子节点的level是0

      newroot->slotkey[0] = newkey;  // newkey是根节点分裂后左侧节点的最大key
   ...
   * node inserted into the parent. Unroll / this splitting up to the root.
   ...
   沿着节点向下递归直到叶子节点，在空闲槽位插入键/数据对。
   如果节点溢出（满了），则必须分裂该节点，并将新分裂出的节点插入到父节点中。
   将这种分裂操作逐层回溯到根节点。
  */
  std::pair<iterator, bool> insert_descend(node *n, const key_type &key, const data_type &value,
    ...
    if (!n->isleafnode()) {
      ...
      if (newchild) {
        BTREE_PRINT("btree::insert_descend newchild with key " << newkey << " node " << newchild
                                                               << " at slot " << slot);
        ...
        if (inner->isfull()) {
          ...
          if (slot == inner->slotuse + 1 && inner->slotuse < (*splitnode)->slotuse) {
            ...
            *splitkey = newkey;
            ...
            BTREE_PRINT("btree::insert_descend switching to splitted node " << inner << " slot "
                                                                            << slot);
          }
        }
      ...
      // - 如果不允许重复，且插入点处已经有相同的 key，则插入失败（返回
      // false），不会插入新的数据。
      // - 只有允许重复（multimap/multiset）时，才会插入重复 key。
      ...
      // 叶子节点会在插入前拆分，这样插入时节点总是有空间，插入完成后可能是恰好满的状态
      if (leaf->isfull()) {
        ...
        // 检查插入槽是否在拆分的兄弟节点中
        if (slot >= leaf->slotuse) {
          // 如果插入键在新拆分出来的节点中，则调整插入位置。
          slot -= leaf->slotuse;
          leaf = static_cast<leaf_node *>(*splitnode);  // leaf 始终指向插入节点
        }
      }
  ...
  /**
   * @brief 将一个内部节点拆分为两个大小相等的兄弟节点。
   *
   * @param inner
   * @param _newkey 返回本叶子节点分裂后，左侧叶子节点（地址与原节点相同）的最大键
   * @param _newinner 返回指向新创建的右侧叶子节点的指针
   * @param addslot 要插入项的槽位，以便在插入后节点大小相同
   */
  void split_inner_node(inner_node *inner, key_type *_newkey, node **_newinner,
    ...
    BTREE_PRINT("btree::split_inner_node on " << inner << " into two nodes " << mid << " and "
 ...
 public:
  ...
  void bulk_load(Iterator ibegin, Iterator iend) {
    ...
    BTREE_PRINT("btree::bulk_load, level 0: "
                << m_stats.itemcount << " items into " << num_leaves << " leaves with up to "
    ...
    for (int level = 2; num_parents != 1; ++level) {
      ...
      BTREE_PRINT("btree::bulk_load, level " << level << ": " << num_children << " children in "
 ...
 private:
  ...
  struct result_t {
    ...
    inline result_t(result_flags_t f, const key_type &k) : flags(f), lastkey(k) {}
 ...
 public:
  ...
  void erase(iterator iter) {
    BTREE_PRINT("btree::erase_iter(" << iter.currnode << "," << iter.currslot << ") on btree size "
                                     << size());
 ...
 private:
   ...
   * merging two sibling nodes or trimming the tree.
   沿着树向下查找指定的键。
   在查找过程中，父节点、左右兄弟节点及它们的父节点都会被计算并传递下去。
   一旦找到对应的键/数据对，就会从叶子节点中移除它。

   如果叶子节点发生下溢（元素不足），会有6种不同的处理情况。

   这些情况通过从相邻兄弟节点移动键/数据对、合并两个兄弟节点或修剪树来解决下溢问题。
   */
  /**
   * @brief 删除B+树中与键匹配的一个（第一个）键/数据对。
   *
   * @param key
   * @param curr
   * @param left
   ...
   * @return result_t
   */
  result_t erase_one_descend(const key_type &key, node *curr, node *left, node *right,
    ...
    if (curr->isleafnode()) {
      ...
      // 如果叶子节点的最后一个键被更改，父节点会收到通知并更新该叶子节点的键
      if (slot == leaf->slotuse) {
      ...
      if (leaf->isunderflow() && !(leaf == m_root && leaf->slotuse >= 1)) {
        ...
        // with our parent
        // 情况2: 如果左叶子节点和右叶子节点在进行平衡时都会发生下溢，则需要进行合并。
        // few指的是节点中包含的键的数量过少（小于阈值的是下溢，小于或等于阈值的都认为是few）
        else if ((leftleaf == NULL || leftleaf->isfew()) &&
                 (rightleaf == NULL || rightleaf->isfew())) {
          if (leftparent == parent) {
            // 如果左侧兄弟节点 和 当前节点 有相同的父节点，则与左侧兄弟节点合并
        ...
        // case : both the left and right leaves have extra data and our
        // parent, choose the leaf with more data
        // 情况5:
        // 两侧的兄弟节点都有额外的数据，并且有相同的父节点，选择数据更多的叶子节点进行平衡
        ...
        // 情况6: 两侧的兄弟节点都有额外的数据，但没有相同的父节点。
        // 如果左侧兄弟节点有相同的父节点，则与左侧兄弟节点平衡，否则与右侧兄弟节点平衡
        else {
          if (leftparent == parent) {
            shift_right_leaf(leftleaf, leaf, leftparent, parentslot - 1);
          } else {
      ...
      // {  // 输出当前处理的inner节点的所有key
      //   auto slotuse = inner->slotuse;
      //   for (auto i = 0; i < slotuse; i++) {
      //     printf("Inner node key %d: %d\n", i, inner->slotkey[i]);
      //   }
      // }

      if (slot == 0) {
        myleft =
            (left == NULL) ? NULL : (static_cast<inner_node *>(left))->childid[left->slotuse - 1];
      ...
      // { std ::cout << "xz erase_one_descend into " << inner->childid[slot] << std ::endl; }

      result_t result = erase_one_descend(key, inner->childid[slot], myleft, myright, myleftparent,
                                          myrightparent, inner, slot);
      ...
      if (result.has(btree_fixmerge)) {
        ...
        // 如果当前节点是叶子节点的父节点，且被合并的子节点是叶子节点
        // 那么这个节点需要更新被合并子节点的最大键
        if (inner->level == 1) {
      ...
      if (inner->isunderflow() && !(inner == m_root && inner->slotuse >= 1)) {
        ...
        // a shift, then merging is necessary. choose the more local merger
        // with our parent
        // 情况2: 如果左侧和右侧的兄弟节点在进行平衡时都会发生下溢，则需要进行合并。
        // 选择与当前节点有相同父节点的兄弟节点进行合并
        ...
        // case : the right leaf has extra data, so balance right with current
        // 情况3: 右侧兄弟节点有额外的数据，因此与当前节点进行右侧平衡
        else if ((leftinner != NULL && leftinner->isfew()) &&
                 (rightinner != NULL && !rightinner->isfew())) {
          if (rightparent == parent) {
        ...
        // case : the left leaf has extra data, so balance left with current
        // 情况4: 左侧兄弟节点有额外的数据，因此与当前节点进行左侧平衡
        else if ((leftinner != NULL && !leftinner->isfew()) &&
                 (rightinner != NULL && rightinner->isfew())) {
          if (leftparent == parent) {
        ...
        // parent, choose the leaf with more data
        // 情况5: 左右兄弟节点都有额外的数据，并且有相同的父节点，选择数据更多的叶子节点进行平衡
        else if (leftparent == rightparent) {
        ...
        // case 6: 左右兄弟节点都有额外的数据，但没有相同的父节点。
        // 如果左侧兄弟节点有相同的父节点，则与左侧兄弟节点平衡，否则与右侧兄弟节点平衡
        else {
          if (leftparent == parent) {
            shift_right_inner(leftinner, inner, leftparent, parentslot - 1);
   ...
   */
  result_t erase_iter_descend(const iterator &iter, node *curr, node *left, node *right,
                              inner_node *leftparent, inner_node *rightparent, inner_node *parent,
                              unsigned int parentslot) {
    if (curr->isleafnode()) {
      leaf_node *leaf = static_cast<leaf_node *>(curr);
      ...
      if (iter.currslot >= leaf->slotuse) {
        BTREE_PRINT("Could not find iterator (" << iter.currnode << "," << iter.currslot
    ...
    } else  // !curr->isleafnode()
    {
      ...
      while (slot <= inner->slotuse) {
        ...
        result = erase_iter_descend(iter, inner->childid[slot], myleft, myright, myleftparent,
                                    myrightparent, inner, slot);
  ...
  // 合并两个叶子节点。该函数将所有键/数据对从右节点移动到左节点，并将右节点的slotuse设置为零。
  // 然后，调用父节点删除右节点的槽。
  result_t merge_leaves(leaf_node *left, leaf_node *right, inner_node *parent) {
    ...
    // printf("Merge leaf nodes %p and %p with common parent %p.\n", left, right, parent);
  ...
  /// removed by the calling parent node.
  static result_t merge_inner(inner_node *left, inner_node *right, inner_node *parent,
                              unsigned int parentslot) {
  ...
  static result_t shift_left_leaf(leaf_node *left, leaf_node *right, inner_node *parent,
    ...
    BTREE_PRINT("Shifting (leaf) " << shiftnum << " entries to left " << left << " from right "
  ...
  /// updated if possible.
  static void shift_left_inner(inner_node *left, inner_node *right, inner_node *parent,
                               unsigned int parentslot) {
    BTREE_ASSERT(left->level == right->level);
    BTREE_ASSERT(parent->level == left->level + 1);
    ...
    BTREE_PRINT("Shifting (inner) " << shiftnum << " entries to left " << left << " from right "
                                    << right << " with common parent " << parent << ".");
  ...
  /// if possible.
  static void shift_right_leaf(leaf_node *left, leaf_node *right, inner_node *parent,
                               unsigned int parentslot) {
    BTREE_ASSERT(left->isleafnode() && right->isleafnode());
    BTREE_ASSERT(parent->level == 1);
    ...
    BTREE_PRINT("Shifting (leaf) " << shiftnum << " entries to right " << right << " from left "
  ...
  /// right so that both nodes are equally filled. The parent node is updated
  /// if possible.
  static void shift_right_inner(inner_node *left, inner_node *right, inner_node *parent,
                                unsigned int parentslot) {
    BTREE_ASSERT(left->level == right->level);
    BTREE_ASSERT(parent->level == left->level + 1);
 ...
 private:
  ...
  static void print_node(std::ostream &os, const node *node, unsigned int depth = 0,
  ...
  struct dump_header {
    ...
    inline bool same(const struct dump_header &o) const {
      return (signature[0] == 's' && signature[1] == 't' && signature[2] == 'x' &&
             ...
             (innerslots == o.innerslots) && (allow_duplicates == o.allow_duplicates);
    }
  };
