...
class cuckoohash_map {
 ...
 public:
   ...
   */
  cuckoohash_map(size_t n = DEFAULT_SIZE, double mlf = DEFAULT_MINIMUM_LOAD_FACTOR,
                 size_t mhp = NO_MAXIMUM_HASHPOWER, const hasher &hf = hasher(),
  ...
  typename std::enable_if<std::is_convertible<Updater, updater_type>::value, bool>::type update_fn(
  ...
  typename std::enable_if<std::is_convertible<Updater, updater_type>::value, void>::type upsert(
      K &&key, Updater fn, Args &&...val) {
    size_t hv = hashed_key(key);
    cuckoo_status st;
    do {
      auto b = snapshot_and_lock_two(hv);
 ...
 private:
  ...
  struct BucketContainer {
    static_assert(N >= 1 && N <= 3,
                  "BucketContainer should only be used"
  ...
  //
  // throws hashpower_changed if it changed after taking the lock.
  std::pair<TwoBuckets, OneBucket> lock_three(const size_t hp, const size_t i1, const size_t i2,
    ...
    return std::make_pair(
        ...
        OneBucket{(lock_ind(i3) == lock_ind(i1) || lock_ind(i3) == lock_ind(i2)) ? nullptr : this,
                  i3});
  }
    ...
    while (!q.full() && !q.empty()) {
      ...
      for (size_t i = 0; i < slot_per_bucket && !q.full(); ++i) {
        ...
        if (x.depth < MAX_BFS_PATH_LEN - 1) {
          b_slot y(alt_index(hp, partial, x.bucket), x.pathcode * slot_per_bucket + slot,
                   x.depth + 1);
          q.enqueue(y);
        }
      }
  ...
  //
  // throws hashpower_changed if it changed during the search
  int cuckoopath_search(const size_t hp, CuckooRecords &cuckoo_path, const size_t i1,
                        const size_t i2) {
  ...
  bool try_read_from_bucket(const partial_t partial, const key_type &key, mapped_type &val,
  ...
  // for use afterwards.
  template <typename K, typename... Args>
  void add_to_bucket(const partial_t partial, Bucket &b, const size_t slot, K &&key,
                     Args &&...val) {
  ...
  bool try_find_insert_bucket(const partial_t partial, const key_type &key, const Bucket &b,
                              int &slot) const {
    slot = -1;
  ...
  cuckoo_status cuckoo_insert(const size_t hv, TwoBuckets b, K &&key, Args &&...val) {
    ...
    if (st == failure_under_expansion) {
      ...
      add_to_bucket(partial, buckets_[insert_bucket], insert_slot, std::forward<K>(key),
  ...
  cuckoo_status cuckoo_delete(const key_type &key, const size_t hv, const size_t i1,
                              const size_t i2) {
    const partial_t partial = partial_key(hv);
  ...
  // function.
  template <typename V>
  cuckoo_status cuckoo_update(const size_t hv, const size_t i1, const size_t i2,
  ...
  template <typename Updater>
  cuckoo_status cuckoo_update_fn(const key_type &key, Updater fn, const size_t hv, const size_t i1,
                                 const size_t i2) {
    const partial_t partial = partial_key(hv);
  ...
  void move_buckets(size_t current_hp, size_t new_hp, size_t start_lock_ind, size_t end_lock_ind) {
    for (; start_lock_ind < end_lock_ind; ++start_lock_ind) {
      for (size_t bucket_i = start_lock_ind; bucket_i < hashsize(current_hp);
  ...
  // new one.
  static void insert_into_table(cuckoohash_map<Key, T, Hash, Pred, Alloc, slot_per_bucket> &new_map,
  ...
  cuckoo_status cuckoo_expand_simple(size_t new_hp, bool is_expansion) {
    ...
    cuckoohash_map<Key, T, Hash, Pred, Alloc, slot_per_bucket> new_map(hashsize(new_hp) *
                                                                       slot_per_bucket);
  ...
  class locked_table {
   ...
   private:
    ...
    class templated_iterator : public std::iterator<std::bidirectional_iterator_tag, value_type> {
      ...
      // over.
      std::reference_wrapper<typename std::conditional<IS_CONST, const buckets_t, buckets_t>::type>
          buckets_;
 ...
 private:
  ...
  std::vector<cacheint, typename allocator_type::template rebind<cacheint>::other> num_inserts_,
      num_deletes_;
