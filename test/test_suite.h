 ...
 * random number)
 */
template <uint64_t lower, uint64_t upper> class SimpleInt64Random {
public:
  /*
...
private:
  ...
  const char *event_name_list[EVENT_COUNT] = {
      "PAPI_LD_INS", "PAPI_L1_LDM", "PAPI_SR_INS",
      "PAPI_L1_STM", "PAPI_L3_TCA", "PAPI_L3_TCM",
  };
...
public:
  ...
  void PrintL3CacheUtilization() {
    ...
    std::cout << "    L3 total = " << l3_util.first
              << "; miss = " << l3_util.second << "; hit ratio = "
  ...
  void PrintL1CacheUtilization() {
    ...
    std::cout << "    LOAD/STORE total = " << l1_util.first
              << "; miss = " << l1_util.second << "; hit ratio = "
...
// Benchmark for ART
void BenchmarkARTSeqInsert(ARTType *t, int key_num, int num_thread,
                           long int *array);
